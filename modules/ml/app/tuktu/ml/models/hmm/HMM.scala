...
class HMM(numHidden: Int, numObserved: Int) extends BaseModel() {
     ...
     */
    private def RunBackward(observations: List[Int]): List[List[Double]] = {
        var probs = collection.mutable.ListBuffer[collection.mutable.ListBuffer[Double]]()
     ...
     */
    private def ComputeGamma(i: Int, t: Int, observations: List[Int], fwd: List[List[Double]], bwd: List[List[Double]]): Double = {
     ...
     */
    private def ComputeProb(t: Int, i: Int, j: Int, observations: List[Int], fwd: List[List[Double]], bwd: List[List[Double]]): Double = {
        val numerator =
            if (t == observations.size - 1)
                fwd(i)(t) * A(i)(j)
            else
                fwd(i)(t) * A(i)(j) * B(j)(observations(t + 1)) * bwd(j)(t + 1)
