 ...
 * https://github.com/josepablocam/spark-timeseries https://github.com/josepablocam/spark-timeseries/tree/arima
 */
...
object ARIMA {
    ...
    private def HannanRisannenInit(
        ...
        val estimated = arTerms1.zip(
            Array.fill(yTrunc.length)(arModel.coefficients)).map { case (v, b) => v.zip(b).map { case (yi, bi) => yi * bi }.sum + arModel.c }
...
class ARIMAModel(
    ...
    def logLikelihoodCSSARMA(diffedY: Array[Double]): Double = {
        ...
        val css = diffedY.zip(yHat.toArray).drop(maxLag).map {
            case (obs, pred) =>
                math.pow(obs - pred, 2)
        }.sum
        val sigma2 = css / n
    ...
    private def iterateARMA(
        ...
        initErrors: Array[Double] = null): Vector[Double] = {
