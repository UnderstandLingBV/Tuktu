 ...
 * 
 * Adapted to deal with continuous (Double) inputs
 */

import scala.util.Random
import scala.math

class cdA(val N: Int, val n_visible: Int, val n_hidden: Int,
    ...
    def get_corrupted_input(x: Array[Double], tilde_x: Array[Double], p: Double) {
        ...
        for (i <- 0 until n_visible) {
            if (x(i) == 0)
                ...
                tilde_x(i) = if (binomial(1, p) == 1) 0.0 else x(i)
        }
    }
    ...
    def train(x: Array[Double], lr: Double, corruption_level: Double) {
        var i: Int = 0
        var j: Int = 0
