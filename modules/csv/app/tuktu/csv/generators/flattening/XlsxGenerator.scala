...
class XlsxReader(parentActor: ActorRef, fileName: String, sheetName: String, valueName: String,
    ...
    def receive() = {
        case ip: InitPacket => {
            ...
            val workbook = new XSSFWorkbook(new FileInputStream(new File(fileName)))
            // Get the proper sheet
            val sheet = workbook.getSheet(sheetName)
            val rowIterator = sheet.iterator

            // Start processing
            self ! new XlsxReadPacket(workbook, rowIterator, 0)
        }
        case sp: XlsxStopPacket => {
            // Close XLS reader, kill parent and self
            sp.workbook.close
            parentActor ! new StopPacket
        }
        case pkt: XlsxReadPacket => pkt.reader.hasNext match {
            case false => self ! new XlsxStopPacket(pkt.workbook) // EOF, stop processing
            case true => {
                ...
                if (line(endFieldCol) == endField) {
                    ...
                    self ! new XlsxStopPacket(pkt.workbook)
                } else {
                    ...
                    self ! new XlsxReadPacket(pkt.workbook, pkt.reader, pkt.rowOffset + 1)
                }
            }
        }
    }
