...
class ParsingTests extends FlatSpec with Matchers {
    ...
    "ArithmeticParser" should "parse numbers" in {
    ...
    "ArithmeticParser" should "do some basic arithmetic" in {
    ...
    "ArithmeticParser" should "honor operation order" in {
        ArithmeticParser("1.7e2 - 1.8e1 * 2 + 12") should be(1.7e2 - 1.8e1 * 2 + 12)
        ArithmeticParser("1.7e2 - 1.8e1 / 2 - 12") should be(1.7e2 - 1.8e1 / 2 - 12)
    }

    "ArithmeticParser" should "honor bracket order" in {
        ArithmeticParser("17 + (1.7e2 - 1.8e1) * (2 + 7) - 23") should be(17 + (1.7e2 - 1.8e1) * (2 + 7) - 23)
        ArithmeticParser("17 + (1.7e2 - 1.8e1) / (2 + 7) - 23") should be(17 + (1.7e2 - 1.8e1) / (2 + 7) - 23)
    }

    "ArithmeticParser" should "support nested brackets" in {
        ArithmeticParser("((17)) + ((1.7e2 - 1.8e1) * (2 + 7) - (23))") should be(((17)) + ((1.7e2 - 1.8e1) * (2 + 7) - (23)))
    }

    "ArithmeticParser" should "ignore white space" in {
        ArithmeticParser("  1e3  ") should be(1e3)
        ArithmeticParser(" 17 + ( 1.7e2 - 1.8e1 ) * ( 2 + 7 ) - 23 ") should be(17 + (1.7e2 - 1.8e1) * (2 + 7) - 23)
        ArithmeticParser(" 17 + ( 1.7e2 - 1.8e1 ) / ( 2 + 7 ) - 23 ") should be(17 + (1.7e2 - 1.8e1) / (2 + 7) - 23)
    }
    ...
    "TuktuArithmeticsParser" should "return correct results for its functions on random data" in {
    ...
    "PredicateParser" should "evaluate literal expressions correctly" in {
    ...
    "PredicateParser" should "support multiple negation of literals" in {
        PredicateParser("!(!(true))") should be(true)
        PredicateParser("!(!true)") should be(true)
        PredicateParser("!!true") should be(true)
        PredicateParser("!!!(!!(!!true))") should be(false)
    ...
    "PredicateParser" should "support basic predicates" in {
    ...
    "PredicateParser" should "support number comparisons" in {
    ...
    "PredicateParser" should "support string comparisons" in {
    ...
    "TuktuPredicateParser" should "return correct results for its functions" in {
        val datum = Map(
